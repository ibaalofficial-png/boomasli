...
const Equalizer = {
    ...
    init(sharedContext, externalSourceNode){
        if(this.initialized) {
        ...
        try {
            this.context = sharedContext || new (window.AudioContext || window.webkitAudioContext)();
            const audioElement = document.getElementById("audio");
            
            this.source = externalSourceNode || this.context.createMediaElementSource(audioElement);
...
document.addEventListener("change", (e) => {
    if (e.target && e.target.id === "customEqSelect") {
        Equalizer.setCustomPreset(e.target.value);
    }
});
